import CustomizedTable from "../../component/table/table";
import PeopleAltIcon from "@mui/icons-material/PeopleAlt";

import { NavLink, Route, Routes } from "react-router-dom";
import AddCircleIcon from "@mui/icons-material/AddCircle";
import AcadmyTable from "../../component/table/acadmyTable/AcadmeyTabel";
import EmploeeTable from "../../component/table/EmploeeTable/EmploeeTable";
import StudentRateTable from "../../component/studentBage/StudentRateTable/StudentRateTable";
import SubscriptionTable from "../../component/studentBage/Subscription/SubscriptionTable";
import CartTable from "../../component/studentBage/Cart/CartTable";
const Cart = () => {
  return (
    <div className="all-tab-top all-info-top-header main-info-top">
      <div className="TablePageHeader">
        <div className="HeaderContainer">
          <div className="info-content-header d-flex align-items-center justify-content-between gap-3 flex-wrap w-100">
            <div className="d-flex align-items-center name">
              <div className="icon">
                <PeopleAltIcon sx={{ color: "#A3AED0" }} />
              </div>
              <div style={{ color: "#7E8799" }}>العربات</div>
            </div>
            <div className="userTabs">
              <ul className="flex-wrap gap-3">
                <NavLink
                  to={"/admin/Cart"}
                  end
                  className={({ isActive }) => {
                    return isActive ? " tablePage active" : "tablePage";
                  }}
                >
                  عربات التسوق (100)
                </NavLink>
                <NavLink
                  to={"/admin/Cart/full"}
                  className={({ isActive }) => {
                    return isActive ? " tablePage active" : "tablePage";
                  }}
                >
                  العربات الممتلئة
                </NavLink>
              </ul>
            </div>
            <div className=""></div>
          </div>
        </div>
      </div>
      <Routes>
        <Route path="/" element={<CartTable />} />
        <Route path="/full" element={<CartTable />} />
      </Routes>
    </div>
  );
};

export default Cart;
